...
import { 
  ...
  StrokesGainedStats,
  CourseInfo,
  TournamentCourseData,
  HoleStatistic,
  generateCourseIdFromName
} from './types/stats';
 ...
 */
export async function scrapeCourseInfo(
  ...
  try {
    ...
    const courseInfo = await page.evaluate(() => {
      ...
      function getTextContentFromMultipleSelectors(selectors: string[]): string | null {
      ...
      function extractNumberFromText(text: string | null): number | null {
        if (!text) return null;
      ...
      return {
        ...
        par: par,
        yardage: yardage,
        courseRating: null as number | null, // Not available on PGA Tour course stats page
        slopeRating: null as number | null, // Not available on PGA Tour course stats page
        courseType: courseType,
        elevation: null as number | null, // Would need elevation API
        designer: designer,
        yearBuilt: yearBuilt,
 ...
 */
export async function scrapeMultipleCourses(
